        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <p style="margin-left: 20px">
                    <button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#addRoute">Add Price</button>
                    <button type="button" class="btn btn-warning btn-lg">Edit Price</button>
                    <button type="button" class="btn btn-danger btn-lg" data-toggle="modal" data-target="#delPrice">Delete Price</button>
                </p>
            </div>
            <!-- /.row -->
            <div class="row">
                <div class="col-lg-12">
                    <div>
                        ...
                        <table class="table table-hover">
                            ...
                            <?php
                            include '../db.php';
                            $results = mysql_query("SELECT route_id, class, price FROM price");
...
<div class="modal fade" id="delPrice" tabindex="-1" role="dialog" aria-labelledby="myDelPrice">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            ...
            <div class="modal-body" id="price_form">
                <form  method="post" id="priceform" action="delprice.php">
                    <div class="form-group">
                        <label for="route">Price List</label>
                        <select class="form-control" name="delpric" form="priceform">
                            <?php
                            include '../db.php';
                            $results = mysql_query("SELECT price_id, CONCAT(route_id,'-',class,'-',price) as srcdest FROM price");
                            while($rows = mysql_fetch_array($results))
                            {
                                echo '<option value="'.$rows['price_id'].'">'.$rows['srcdest'].'</option>';
                            }
                            ?>
                        </select>
                    </div>
                    <Button type="submit" id="submit" class="btn btn-success">Delete Price</Button>
                </form>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
